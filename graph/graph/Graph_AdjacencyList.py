...
from graph.list.DoubleLinkedList import ListaDoppiamenteCollegata as List
...
class GraphAdjacencyList(GraphBase):
    ...
    def addNode(self, value, weight):
        ...
        """
        newnode = super().addNode(value, weight)  # create a new node with the correct ID
    ...
    def getMaxWeightNode(self):
        """

        Return the max weight of node, if exist.
        :return: the node, if exists; None, otherwise.
        """
        maxWeight = -1
        id_node_max = None
        ...
        for node in nodes:
            if node.weight > maxWeight:
                ...
                id_node_max = node.id
        return self.getNode(id_node_max)
    ...
    def insertEdge(self, node1, node2):
        ...
        if node1 in self.nodes and node2 in self.nodes:  # TODO overwrite if edge already exists
            ...
            self.adj[node2].addAsLast(node1)
